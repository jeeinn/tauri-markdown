...
### Added
- **Outline Sidebar Resize**: Drag the outline sidebar edge to adjust width (180–500px), with preference persisted across sessions
- **Long Outline Titles**: Horizontal scroll and hover tooltip for full heading text when titles exceed sidebar width

### Fixed
- **Outline Rendering**: Fixed blank outline panel caused by resize handle interfering with Vditor's content container
...
### Fixed
- **Ctrl+F Search**: Fixed Ctrl+F shortcut not finding results because `vditorRef` was not set before opening Find/Replace panel
...
### Fixed
- **Find/Replace Content Detection**: Added content change detection for find/replace operations to properly track modifications
- **Updater Progress Display**: Replaced progress placeholder in updater download progress display for accurate percentage showing

### Changed
- **Notification Types Optimization**: Refactored and optimized notification types for better code organization
...
[0.9.1]: https://github.com/jeeinn/tauri-markdown/compare/v0.9.0...v0.9.1

...
### 新增功能
- **PDF 导出**: 将 Markdown 导出为 PDF，保留格式
- **HTML 导出**: 将 Markdown 导出为 HTML，方便网页分享
- **拖拽打开文件**: 直接拖拽 .md/.markdown/.txt 文件到应用中打开
- **滚动位置记忆**: 重新打开文档时自动恢复滚动位置（可开关）

### 问题修复
- **网络图片 403**: 添加 no-referrer 策略解决第三方图片加载问题
- **导出功能**: 修复导出时的国际化配置和相对路径图片处理

### 改进优化
- **菜单系统**: 重构菜单结构，添加嵌套子菜单和分组标题，组织更清晰
- **测试框架**: 迁移到 Vitest，提升测试覆盖率
- **代码组织**: 模块化滚动记忆和导出工具函数
